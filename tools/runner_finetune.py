...
from datasets.ModelNetDataset import pc_normalize
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if args.use_gpu:
        base_model.to(args.local_rank)     # Put Model onto GPU
    # DDP
    if args.distributed:
        # Sync BN
        if args.sync_bn:
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            fps_idx = pointnet2_utils.furthest_point_sample(points.contiguous(), point_all)  # (B, npoint)
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if better:
                ...
                print_log(
                    ...
                    logger=logger)
            if args.vote:
                if metrics.acc > 91.4 or (better and metrics.acc > 90.9):
