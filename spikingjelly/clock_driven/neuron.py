...
class BaseNode(base.MemoryModule):
    ...
    def neuronal_reset(self, spike, x):
        """
...
class MultiStepIFNode(IFNode):
    def __init__(self, timestep, v_threshold: float = 1., v_reset: float = 0.,
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        if x_seq.shape[0] != self.T:
            x_seq =x_seq.view(self.T,-1,*x_seq.shape[1:])
            fusion = True
        ...
        if self.backend == 'torch':
            ...
            return spike_seq if not fusion else spike_seq.flatten(0,1)
...
class MultiStepLIFNode(LIFNode):
    def __init__(self, timestep, tau: float = 2., decay_input: bool = True, v_threshold: float = 0.5,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
        ...
        * :ref:`中文API <MultiStepLIFNode.__init__-cn>`
        ...
        # 注册一个名为 'v_seq' 的内存变量，初始值为 None
        self.register_memory('v_seq', None)

        # 检查 backend 是否合法
        check_backend(backend)

        # 将 backend 赋值给 self.backend
        self.backend = backend
        self.T = timestep
        self.lava_s_cale = lava_s_cale
        # 将 lava_s_cale 赋值给 self.lava_s_cale
...
class ParametricLIFNode(BaseNode):
    def __init__(self, init_tau: float = 2.0, decay_input: bool = True, v_threshold: float = 1.,
        ...
        * :ref:`中文API <ParametricLIFNode.__init__-cn>`
        ...
        self.w._no_weight_decay = True  # Not Weight-decay
...
class MultiStepParametricLIFNode(ParametricLIFNode):
    def __init__(self, timestep, init_tau: float = 2., decay_input: bool = True, v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
...
class ManLIFNode(BaseNode):
    def __init__(self, init_tau: float = 2.0, decay_input: bool = True, v_threshold: float = 1.,
        ...
        self.A._no_weight_decay = True  # Not Weight-decay
        self.B._no_weight_decay = True  # Not Weight-decay
        self.C._no_weight_decay = True  # Not Weight-decay
        self.D._no_weight_decay = True  # Not Weight-decay
    ...
    def extra_repr(self):
        with torch.no_grad():
            ...
            C = self.C
            D = self.D
        return super().extra_repr() + f', A={A}, B={B}, C={C}, D={D}'

    def neuronal_charge(self, x: torch.Tensor):
        if self.decay_input:
            if self.v_reset is None or self.v_reset == 0.:
                self.v = self.v * self.A.sigmoid() + x * self.B.sigmoid()
            else:
                self.v = self.v * self.A.sigmoid() + (x + self.v_reset) * self.B.sigmoid()
        else:
            pass
    ...
    def neuronal_reset(self, spike, x):
        ...
        if self.v_reset is None:
            # soft reset
            self.v = (self.C * self.v + self.D * x) - spike_d * self.v_threshold

        else:
            # hard reset
            self.v = (1. - spike_d) * (self.C * self.v + self.D * x) + spike_d * self.v_reset
   

class MultiStepManLIFNode(ManLIFNode):
    def __init__(self, timestep, init_tau: float = 2., decay_input: bool = True, v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False, backend='torch'):
        super().__init__(init_tau, decay_input, v_threshold, v_reset, surrogate_function, detach_reset)
...
class MultiStepEIFNode(EIFNode):
    def __init__(self, timestep, tau: float = 2., delta_T: float = 1., theta_rh: float = .8, v_threshold: float = 1., v_rest: float = 0., v_reset: float = -0.1,
