    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        if constexpr (kHasZ) {
            ...
            for (int i = 0; i < kNItems; ++i) {
                ...
                dz_vals[i] = dout_vals[i] * float(out_vals[i]);
                dout_vals[i] *= float(z_vals[i]);  
            }
            // if (blockIdx.x == 0 && blockIdx.y == 0) {
            //     printf("dout_vals=%f, dz_vals = %f\n", dout_vals, dz_vals);
            // }
            __syncthreads();
            ...
            if (params.out_z_ptr != nullptr) {  // Recompute and store out_z
                ...
                for (int i = 0; i < kNItems; ++i) { out_z_vals[i] = float(out_vals[i]) * float(z_vals[i]); }
