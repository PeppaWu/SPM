...
class _MultiStepThresholdDependentBatchNormBase(_BatchNorm):
    def __init__(self, alpha: float, v_th: float, *args, **kwargs):
        ...
        torch.nn.init.constant_(self.weight, alpha * self.v_th)

    def forward(self, x_seq):
        y = super().forward(x_seq)
        return y
...
class MultiStepTemporalWiseAttention(nn.Module):
    def __init__(self, T: int, reduction: int = 16, dimension: int = 4):
        ...
        * :ref:`中文API <MultiStepTemporalWiseAttention.__init__-cn>`
        ...
        """
        super().__init__()
        assert dimension == 4 or dimension == 3, 'dimension must be 4 or 3'
        ...
        self.sharedMLP = nn.Sequential(
            nn.Linear(T, int(T / reduction), bias=False),
            nn.ReLU(),
            nn.Linear(int(T / reduction), T, bias=False)
        )

        self.sigmoid = nn.Sigmoid()

    def forward(self, x_seq: torch.Tensor):
        assert x_seq.dim() == 4 or x_seq.dim() == 5, ValueError(f'expected 4D or 5D input with shape [T, N, M, C] or [T, N, C, H, W], but got input with shape {x_seq.shape}')
